(function () {
 ...
 var tableHeadings = ["Student", "Email", "Profile picture", "Skils", "Controls"];


 for (let i = 0; i < tableHeadings.length; i++) {
     var th = document.createElement('th');
     var content = document.createTextNode(tableHeadings[i]);
     th.appendChild(content);
     if (tableHeadings[i] !== "Controls" && tableHeadings[i] !== "Profile picture") {
 ...
 var filterStudents = students.map(function(stud) {
     return {
 ...
 function addStudents() {
     for (var i = 0; i < filterStudents.length; i++) {
         ...
         var studTd = document.createElement("td");
         var studList = document.createTextNode(filterStudents[i].student);
         studTd.appendChild(studList);
         tr.appendChild(studTd);

         var studMail = document.createElement("td");
         var mailList = document.createTextNode(filterStudents[i].email);
         studMail.appendChild(mailList);
         tr.appendChild(studMail);

         var studImg = document.createElement("td");
         var img = document.createElement("img");
         img.setAttribute("src", filterStudents[i].img);
         img.style.width = '130px';
         studImg.appendChild(img);
         tr.appendChild(studImg);

         var studSkills = document.createElement("td");
         var skillsList = document.createTextNode(filterStudents[i].skills);
         studSkills.appendChild(skillsList);
         tr.appendChild(studSkills);

         var studControls = document.createElement("td");
         var edit = document.createElement("button");
         edit.classList.add("glyphicon", "glyphicon-edit");
         var remove = document.createElement("button");
         remove.classList.add("glyphicon", "glyphicon-trash");
         studControls.appendChild(remove);
         studControls.appendChild(edit);
         tr.appendChild(studControls);
 ...
 function deleteStudent() {
     var deleteStud = document.getElementsByClassName('glyphicon-trash');
     for (let i = 0; i < deleteStud.length; i++) {
         deleteStud[i].onclick = function() {
             filterStudents.splice(i, 1);
 ...
 function alertStudent(event) {
     ...
     clicked = clicked.parentNode;
     alert(clicked.firstChild.innerHTML)
 }
 tbody.addEventListener("click", alertStudent);

 function createForm() {
     var formHeadings = ['Name', 'Last name', 'Email', 'Profile picture', 'Skills'];
     var form = document.createElement('form');
     form.style.margin = '10px 0';

     formHeadings.forEach(function(name) {
     ...
     saveBtn.setAttribute("class", "saveBtn btn btn-primary");
     saveBtn.style.margin = '15px 15px 0 0';

     var cancelBtn = document.createElement('button');
     var cancelBtnText = document.createTextNode('Cancel');
     cancelBtn.appendChild(cancelBtnText);
     form.appendChild(cancelBtn);
     cancelBtn.setAttribute("class", "cancelBtn btn btn-default");
     cancelBtn.style.margin = '15px 15px 0 0';
